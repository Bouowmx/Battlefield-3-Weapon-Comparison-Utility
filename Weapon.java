...
final class Weapon {
	...
	public double[] BIPOD, FLASH_SUPPRESSOR, FOREGRIP, HEAVY_BARREL, LASER_SIGHT, LASER_SIGHT_PRIMARY, SUPPRESSOR;
	...
	public void attach(String attachment) {
		...
		if (attachment.equals("Heavy Barrel")) {
			if (HEAVY_BARREL[0] != 0) {DAMAGE_DO_END = HEAVY_BARREL[0];}
			if (HEAVY_BARREL[1] != 0) {MUZZLE_VELOCITY = HEAVY_BARREL[1];}
		...
		if (attachment.equals("Laser Sight")) {
			HIP_STATIONARY_STANDING_SPREAD = LASER_SIGHT[0];
			HIP_STATIONARY_CROUCHING_SPREAD = LASER_SIGHT[0];
			HIP_STATIONARY_PRONE_SPREAD = LASER_SIGHT[0];
			HIP_MOVING_STANDING_SPREAD = LASER_SIGHT[0];
			HIP_MOVING_CROUCHING_SPREAD = LASER_SIGHT[0];
			HIP_MOVING_PRONE_SPREAD = LASER_SIGHT[0];
		}
		if (attachment.equals("Laser Sight (Primary)")) {
			HIP_STATIONARY_STANDING_SPREAD = LASER_SIGHT_PRIMARY[0];
			HIP_STATIONARY_CROUCHING_SPREAD = LASER_SIGHT_PRIMARY[0];
			HIP_STATIONARY_PRONE_SPREAD = LASER_SIGHT_PRIMARY[0];
			HIP_MOVING_STANDING_SPREAD = LASER_SIGHT_PRIMARY[0];
			HIP_MOVING_CROUCHING_SPREAD = LASER_SIGHT_PRIMARY[0];
			HIP_MOVING_PRONE_SPREAD = LASER_SIGHT_PRIMARY[0];
		}
		if (attachment.equals("Suppressor")) {
			DAMAGE_DO_START = SUPPRESSOR[0];
			DAMAGE_DO_END = SUPPRESSOR[1];
			if (SUPPRESSOR[2] != 0) {MUZZLE_VELOCITY = SUPPRESSOR[2];}
